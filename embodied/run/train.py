...
def train(agent, env, replay, logger, args):
    ...
    #     driver(random_agent.policy, steps=2500)
    # sys.exit(0)
    while replay is not None and len(replay) < max(args.batch_steps, args.train_fill):
        driver(random_agent.policy, steps=100)
    logger.add(metrics.result())
    logger.write()

    dataset = agent.dataset(replay.dataset) if replay is not None else None
    state = [None]  # To be writable from train step function below.
    batch = [None]

    def train_step(tran, worker):
        train_steps = should_train(step)
        # print("training for", train_steps, "steps")
        for _ in range(train_steps):
            ...
            if "priority" in outs and replay is not None:
